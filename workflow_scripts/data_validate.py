...
logging.basicConfig(filename="workflow_scripts/readme.md", level=logging.INFO, format='%(message)s')
...
def validate_matches():
    ...
    for match, group in match_group:
        if group['Pick Order'].nunique() != 10:
            logging.error(f'Match {match} does not have 10 heroes!')
        ...
        for i in range(1, 11):
            ...
            if not pick.empty:
                ...
                if current_char in characters_used:
                    logging.error(f'Match {match} has a duplicate character!')
        ...
        if group[group['Team'] == 'My Team']['Team'].value_counts().values[0] != 5 or\
                group[group['Team'] == 'Enemy Team']['Team'].value_counts().values[0] != 5:
                    logging.error(f'Match {match} does not have 5 characters on each teams!')
